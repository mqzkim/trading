...
| 항목 | 값 |
|------|-----|
| Vercel 프로젝트명 | `Codex-workspace` |
| Root Directory | `shipkit` |
| Framework | Next.js |
| Node.js Version | 24.x |
| 플랜 | **Hobby** (무료) |
| 도메인 | `shipkit.work` (Cloudflare) |
| GitHub 연동 | `mqzkim/Codex-workspace` (자동 배포) |
...
| `ANTHROPIC_API_KEY` | all | Codex API |
...
```bash
# 프로덕션 (root에서 실행)
cd /c/workspace/Codex-workspace && npx vercel --prod --yes

# 프리뷰
cd /c/workspace/Codex-workspace && npx vercel --yes
```
...
- 기존 커맨드: `.Codex/commands/vercel-deploy-optimize.md`, `vercel-env-sync.md`, `vercel-edge-function.md`
