...
- 스크립트: `/c/workspace/.claude/hub/scripts/`
...
레지스트리에서 에셋 목록 조회.
```bash
node /c/workspace/.claude/hub/scripts/status.js "" TYPE
```
...
공유 에셋 동기화.
```bash
bash /c/workspace/.claude/hub/scripts/claude-hub-sync.sh --both
```

### `init <repo-path> [alias]`
새 레포를 허브에 등록하고 공유 에셋 부트스트랩.
```bash
bash /c/workspace/.claude/hub/scripts/claude-hub-init.sh <repo-path> [alias]
```
