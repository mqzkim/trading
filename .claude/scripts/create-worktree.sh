...
usage() {
  cat <<'EOF'
Usage:
  .claude/scripts/create-worktree.sh <name> [options]
...
Defaults:
  Worktree root: $CLAUDE_WORKTREE_ROOT or ~/.claude-worktrees/<repo-name>/
  Branch name : codex/claude-<slug>
EOF
}
...
fail() {
  printf '[claude-worktree] ERROR: %s\n' "$*" >&2
  exit 1
}
...
DEFAULT_ROOT="${CLAUDE_WORKTREE_ROOT:-$HOME/.claude-worktrees/$REPO_NAME}"
WORKTREE_PATH="${WORKTREE_PATH:-$DEFAULT_ROOT/$SLUG}"

if [ "$DETACH" -eq 0 ]; then
  BRANCH_NAME="${BRANCH_NAME:-codex/claude-$SLUG}"
fi
...
link_or_copy ".claude/settings.local.json" \
